 ...
 * @author Suthar
 */
public class CMRxBillingNewMono2 extends javax.swing.JFrame {
String baseadd="D:/Shopy/Shopy",logo="src/Icons/logo.png",CMRadd,ITMadd,user,sd1,sd2,sd,cd,cd1,cd2,lang="English";
    String Sname="Unkonwn",ESname="Unkonwn",HSname="Unkonwn",Sno="1234567890";       // Shop Name and CN
    String blbl=" Baki",jlbl = " Jama";
    Boolean imust=false,visiblity=true;String imustc="";
     ...
     */
    public CMRxBillingNewMono2(String add,String golo,String idy, String sy,String ey) {
        baseadd=add;
        logo=golo;
        IDyear = idy;
        Syear = sy;
...
{   
    ...
    if(repeatation.mainn(CMRadd,"customer.txt" ,cd)==1){
      ...
      Balance=0;
      another=0;
      f=0;it=0;reserved=table.getRowCount();
      visiblity=true;
      if(imust){
          visiblity=false;
      }
...
//#######################################################################################################################################
      ...
      Scanner inread ;
      inread = new Scanner(new File(CMRadd+"/"+pcode+"/profile.txt"),"UTF-8");          // to get the name of customer
      inread.useDelimiter("\\n");
      inread.nextLine();
      pEname = inread.nextLine();                // name of customer having code pcode
...
//#######################################################################################################################################
      //int counter =0;
      String dsd=NamingDate.mainn(sd1);
      File tmp1;
      Scanner bal;
      Boolean bg;
      if(DateInBetween.mainn(DtAdd, dsd, sd1)){
      for(;DateInBetween.mainn(DtAdd, dsd, sd1);dsd=datem15.mainn(dsd)){
          tmp1=new File(CMRadd + "/"+pcode+"/Pending/Given/"+NamingDateName.mainn(dsd)+".txt");
          bg = tmp1.exists();
          if(bg){
...
//#######################################################################################################################################
      sd=dsd;
      do{               // for indivisiuale customer it read all possible Given Item files according to date intervals

      String fname = NamingDateName.mainn(sd);                // According to date what name of file should be perfact
      File tempFile = new File( CMRadd + "/"+pcode+"/Pending/Given/"+fname+".txt" );
      
      boolean exists = tempFile.exists();
   if(exists){
      inread = new Scanner(new File(CMRadd + "/"+pcode+"/Pending/Given/"+fname+".txt"),"UTF-8");
      inread.useDelimiter(",|\\n");
      inread.nextLine();
      inread.nextLine();
       ...
       while (inread.hasNext())
{
           ...
           if(DateInBetween.mainn(sd1, pdaate, sd2)){
   ...
   //TextIO.putln(sd);
    sd = datep15.mainn(sd);                 // Go for next Hafta
      }     
      while(DateInBetween.mainn(dsd, sd, sd2));                 // Loop over
...
//sd=sd1;             // So that it also read all files
      sd=dsd;
      another=0;
do{               // for indivisiuale customer it read all possible taken item files according to date intervals

String fname = NamingDateName.mainn(sd);                // According to date what name of file should be perfact
    
File tempFile = new File( CMRadd + "/"+pcode+"/Pending/Taken/"+fname+".txt" );
   boolean exists = tempFile.exists();
   if(exists){
   inread = new Scanner(new File(CMRadd + "/"+pcode+"/Pending/Taken/"+fname+".txt"),"UTF-8");
   inread.useDelimiter(",|\\n");
   inread.nextLine();
...
Ptotal = 0;
   ...
   for(int y =dm.getRowCount()-1 ;y>=reserved;--y){
             ...
             // All customer readed successfully
cd=String.valueOf(1+Integer.valueOf(cd));
}

    } catch (FileNotFoundException ex) {
            JLabel label = new JLabel("CMR profile. problem !!! Error0007");
            label.setFont(new Font("Arial", Font.BOLD, 18));
            JOptionPane.showMessageDialog(null,label,"ERROR",JOptionPane.WARNING_MESSAGE);
                ...
                if(o3==null&&o4==null){   //Name Code Mobile Line
                   // System.out.println("o3==&&o4==");
                    if(o2==null){
                        ...
                        g2d.drawString(String.valueOf(o1),X,y);y+=yShift;
                    }else{if(o1!=null){
                    g2d.drawString(String.valueOf(o1)+"    "+trunct(String.valueOf(o2),145),X,y);y+=yShift;
                    }else{
                        g2d.drawString(String.valueOf(o2),X,y);y+=yShift;
                    }}
                }else{
             if(o4!=null){      //Items
                 g2d.drawString(trunct(String.valueOf(o1),69+extrasp),X,y);//y+=yShift;
                        g2d.drawString(String.format(String.valueOf(o2))+" * "+String.format(String.valueOf(o3))+" = "+String.format(String.valueOf(o4))+"  ",X+extrasp+69,y);y+=yShift;
             }else{
                 if(o2!=null){      //Prevous and Final
                     g2d.drawString(shortFunction.stringc(String.valueOf(o2),36),X+extrasp,y);y+=yShift;
                 }else{
                     if(o3!=null){ //current Len Den Total
                         g2d.drawString(String.valueOf(o3),X+50+extrasp,y);y+=yShift;
                     }
                 }
             }
                 }
          ...
          if(y<my){
             ...
             if(o4!=null){      //Items
                 ...
                 if(o2!=null){      //Last wale Prevous and Finel
                     g2d.drawString(shortFunction.stringc(String.valueOf(o2),36),X,y);y+=yShift;
                 }else{
                     if(o3!=null){ //current Len Den Total
                         g2d.drawString(String.valueOf(o3),X+50,y);y+=yShift;
                     }
                 }
             }
                 }
          ...
          if(y<my){
                ...
                if(o3==null&& o4==null){   //Name Code Mobile Line
                   // System.out.println("o3==&&o4==");
                    if(o2==null){
                        ...
                        if(o1!=null){
                        g2d.drawString(String.valueOf(o1)+"    "+trunct(String.valueOf(o2),145),X,y);y+=yShift; //int width = g2d.getFontMetrics().stringWidth(gh);
                    }else{
                        g2d.drawString(String.valueOf(o2),X,y);y+=yShift;
                    }}
                }else{
             if(o4!=null){      //Items
                 g2d.drawString(trunct(String.valueOf(o1),69),X,y);//y+=yShift;
                        g2d.drawString(String.format(String.valueOf(o2))+" * "+String.format(String.valueOf(o3))+" = "+String.format(String.valueOf(o4))+"  ",X+69,y);y+=yShift;
             }else{
                 if(o2!=null){      //Last wale Prevous and Finel
                     g2d.drawString(shortFunction.stringc(String.valueOf(o2),36),X,y);y+=yShift;
                 }else{
...
System.out.println("\n\n\nAgain for printer");
    ...
    private void settingActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_settingActionPerformed
        ITMxPrint.main(baseadd, logo,IDyear,Syear,Eyear);
...
char c = evt.getKeyChar();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CMRxBillingNewMono2.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CMRxBillingNewMono2("D:/Shopy/Shopy","src/Icons/logo.png","2020-2021","10/01/2020","31/12/2020").setVisible(true);
            }
        });
    }
