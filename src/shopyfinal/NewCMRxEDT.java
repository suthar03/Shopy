...
public class NewCMRxEDT extends javax.swing.JFrame {
...
int amtd=0;int y=0;
    ...
    public NewCMRxEDT(String base,String golo,String idy, String sy,String ey) {
         baseadd=base;
       logo=golo;
        IDyear = idy;
        Syear = sy;
...
ccode.selectAll();
    ...
    public void rep(){
        ...
        if(way.equals("Taken")){
             ...
             if(repeatation.mainn(CMRadd +"/" + ccode.getText()+ STRaddT+"/", NamingDateName.mainn(vtd)+".txt", COsavedform)==1&&repeatation.mainn(ITMadd+"/"+icode.getText()+"/Status", NamingDateName.mainn(vtd)+".txt",IOsavedform)==1){
             try {
            ReplaceLineEx.mainn(CMRadd +"/" + ccode.getText()+ STRaddT+"/", NamingDateName.mainn(vtd)+".txt", COsavedform,CNsavedform,1);
            Scanner in;
                            in = new Scanner(new File(CMRadd +"/" + ccode.getText()+ STRaddT+"/"+ NamingDateName.mainn(vtd)+".txt"),"UTF-8");
                            String pd = in.nextLine();
                            Float nd = Float.valueOf(pd)-striAMT+tot;
                            ReplaceLineWN.mainn(CMRadd+"/"+ccode.getText()+STRaddT, NamingDateName.mainn(vtd)+".txt", 1, String.valueOf(nd));
                            in.close();
                            ...
                            for(dsd = datep15.mainn(dsd);DateInBetween.mainn(dsd, dsd, Eyear);dsd=datep15.mainn(dsd)){                               //Updating all file created or having date of early created
                                tmp = new File(CMRadd +"/" + ccode.getText()+ STRaddG+"/"+ NamingDateName.mainn(dsd)+".txt");
                                bg =tmp.exists();
                                if(bg){
                            ...
                            ReplaceLineWN.mainn(CMRadd +"/" + ccode.getText()+ STRaddG, NamingDateName.mainn(dsd)+".txt",1 , String.valueOf(balnc));
                            in.close();
                                }
                            }
          ...
          JLabel label = new JLabel("Customer data storing date. not found or opens !!! Error0011");
            ...
            JOptionPane.showMessageDialog(null,label,"ERROR",JOptionPane.WARNING_MESSAGE); 
             ...
             Logger.getLogger(NewCMRxEDT.class.getName()).log(Level.SEVERE, null, ex);
             }*/
             
        if(repeatation.mainn(CMRadd +"/" + ccode.getText()+ STRaddG+"/", NamingDateName.mainn(vtd)+".txt", COsavedform)==1&&repeatation.mainn(ITMadd+"/"+icode.getText()+"/Status", NamingDateName.mainn(vtd)+".txt",IOsavedform)==1){
         try {
                Scanner in;
               //if(repeatationWOcase.mainn(CMRadd +"/" + ccode.getText()+ STRaddG,  NamingDateName.mainn(vtd)+".txt", COsavedform)==0&&repeatationWOcase.mainn(ITMadd+"/"+icode.getText()+"/Status", NamingDateName.mainn(vtd)+".txt", IOsavedform)==0)
                 //  System.out.println("You Can ..");
                ReplaceLineEx.mainn(CMRadd +"/" + ccode.getText()+ STRaddG+"/", NamingDateName.mainn(vtd)+".txt", COsavedform,CNsavedform,2);//NamingDateName.mainn(vtd)
                in = new Scanner(new File(CMRadd +"/" + ccode.getText()+ STRaddG+"/"+ NamingDateName.mainn(vtd)+".txt"),"UTF-8");
                            in.nextLine();
                            String pd = in.nextLine();
                            Float nd = Float.valueOf(pd)-striAMT+tot;
                            System.out.println("Updating Total");
                            ReplaceLineWN.mainn(CMRadd+"/"+ccode.getText()+STRaddG, NamingDateName.mainn(vtd)+".txt", 2, String.valueOf(nd));//NamingDateName.mainn(vtd)
                            in.close();
                            String dsd =NamingDate.mainn(vtd);
...
//###################################################################################################################################    
    public void CMRxName(){
        if(repeatation.mainn(CMRadd, "customer.txt", ccode.getText())==1){          //Customer's account exists or not  ####111
...
ccode.requestFocus();
       ...
       DefaultTableModel dm = (DefaultTableModel)table.getModel();             // To clear the table
        ...
        String td;
        File tmp = new File(CMRadd+"/"+ccode.getText()+STRaddG+"/"+NamingDateName.mainn(ndt)+".txt");
        boolean ex = tmp.exists();
        if(ex){
            //System.out.println(CMRadd+"/"+ccode.getText()+STRaddG+"/"+NamingDateName.mainn(ndt)+".txt");
                try {
                    read = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddG+"/" + NamingDateName.mainn(ndt) + ".txt"),"UTF-8");
                    read.useDelimiter(",|\\n");
                    read.nextLine();
                    read.nextLine();
                    while(read.hasNext()){
        ...
        tmp = new File(CMRadd+"/"+ccode.getText()+STRaddT+"/"+NamingDateName.mainn(ndt)+".txt");
        ex = tmp.exists();
        if(ex){
                try {
                    read = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddT+"/" + NamingDateName.mainn(ndt) + ".txt"),"UTF-8");
                    read.useDelimiter(",|\\n");
                    read.nextLine();
                    while(read.hasNext()){
                    td = read.next();
...
//###############################################################################################################################################################3                              
                               ...
                               //System.out.println(CMRadd +"/" + ccode.getText()+ STRaddG+"/"+NamingDateName.mainn(vtd)+".txt\n"+Csavedform);
                               String st1=vtd + "," + ccode.getText() + ",-" + striqty + "," + strirate + "," + striamt;
                               String st2=","+mode+",Customer";
                               String Isavedform=st1+st2;
                               if(repeatation.mainn(CMRadd +"/" + ccode.getText()+ STRaddG, NamingDateName.mainn(vtd)+".txt",Csavedform)==1&&repeatation.mainn(ITMadd+"/"+stricode+"/Status",NamingDateName.mainn(vtd)+".txt",Isavedform)==1){
                           det=true;
                                   RemoveLineEx.mainn(CMRadd +"/" + ccode.getText()+ STRaddG, NamingDateName.mainn(vtd)+".txt", Csavedform,2);
                            in = new Scanner(new File(CMRadd +"/" + ccode.getText()+ STRaddG+"/"+ NamingDateName.mainn(vtd)+".txt"),"UTF-8");
                            in.nextLine();
                            String pd = in.nextLine();
                            ...
                            for(dsd = datep15.mainn(dsd);DateInBetween.mainn(dsd, dsd, Eyear);dsd = datep15.mainn(dsd)){
                               ...
                               bg =tmp.exists();
                               if(bg){
                                in = new Scanner(new File(CMRadd +"/" + ccode.getText()+ STRaddG+"/"+ NamingDateName.mainn(dsd)+".txt"),"UTF-8");   
                                pd = in.nextLine();
                                nd = Float.valueOf(pd)-Float.valueOf(striamt);
                                ReplaceLineWN.mainn(CMRadd+"/"+ccode.getText()+STRaddG, NamingDateName.mainn(dsd)+".txt", 1, String.valueOf(nd));
                                in.close();
                               }
                            }
...
//###################################################################################################################################################################
                                if(repeatation.mainn(CMRadd +"/" + ccode.getText()+ STRaddT, NamingDateName.mainn(vtd)+".txt", Csavedform)==1){
                            det=true;
                                    RemoveLineEx.mainn(CMRadd +"/" + ccode.getText()+ STRaddT, NamingDateName.mainn(vtd)+".txt", Csavedform,1);
                            Scanner in;
                            in = new Scanner(new File(CMRadd +"/" + ccode.getText()+ STRaddT+"/"+ NamingDateName.mainn(vtd)+".txt"),"UTF-8");
                            String pd = in.nextLine();
                            Float nd = Float.valueOf(pd)-Float.valueOf(striamt);
...
if(a==10)
    ...
    //public static void mainn() {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                //new NewCMRxEDT().setVisible(true);
            }
        });
    }
