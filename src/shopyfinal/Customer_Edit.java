...
public class Customer_Edit extends javax.swing.JFrame {
    ...
    String lang = "English";
    String way;
    float striAMT = 0, striQTY = 0;
    int editt, roww;
    int amtd = 0;
    int y = 0;

    String baseadd, logo, CMRadd, ITMadd, user;

    public Customer_Edit(String base, String golo, String idy, String sy, String ey) {
        baseadd = base;
        logo = golo;
        IDyear = idy;
        Syear = sy;
...
//table.setCellSelectionAllowed(false);
    ...
    private void initComponents() {
        ...
        setTitle("Edit Customer's Transaction");
        ...
        hdlbl.setText("-:: Editing Customer's Entry ::-");
    ...
    public void rep() {
        ...
        if (way.equals("Taken")) {
            ...
            INsavedform = COsavedform.substring(0, 10) + "," + ccode.getText() + "," + iqty.getText() + "," + irate.getText() + "," + iamt.getText() + "," + mode + ",Customer";

            if (ShortFunction.repeatation(CMRadd + "/" + ccode.getText() + STRaddT + "/", ShortFunction.NamingDateName(vtd) + ".txt", COsavedform) == 1 && ShortFunction.repeatation(ITMadd + "/" + icode.getText() + "/Status", ShortFunction.NamingDateName(vtd) + ".txt", IOsavedform) == 1) {
                try {
                    ShortFunction.ReplaceLineEx(CMRadd + "/" + ccode.getText() + STRaddT + "/", ShortFunction.NamingDateName(vtd) + ".txt", COsavedform, CNsavedform, 1);
                    Scanner in;
                    in = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddT + "/" + ShortFunction.NamingDateName(vtd) + ".txt"), "UTF-8");
                    String pd = in.nextLine();
                    Float nd = Float.valueOf(pd) - striAMT + tot;
                    ShortFunction.ReplaceLineWN(CMRadd + "/" + ccode.getText() + STRaddT, ShortFunction.NamingDateName(vtd) + ".txt", 1, String.valueOf(nd));
                    in.close();
                    String dsd = ShortFunction.NamingDate(vtd);
                    ...
                    for (dsd = ShortFunction.Date_plus(dsd); ShortFunction.DateInBetween(dsd, dsd, Eyear); dsd = ShortFunction.Date_plus(dsd)) {                               //Updating all file created or having date of early created
                        tmp = new File(CMRadd + "/" + ccode.getText() + STRaddG + "/" + ShortFunction.NamingDateName(dsd) + ".txt");
                        bg = tmp.exists();
                        if (bg) {
                            ...
                            ShortFunction.ReplaceLineWN(CMRadd + "/" + ccode.getText() + STRaddG, ShortFunction.NamingDateName(dsd) + ".txt", 1, String.valueOf(balnc));
                            in.close();
                        }
                    }

                    ShortFunction.ReplaceLineEx(ITMadd + "/" + icode.getText() + "/Status", ShortFunction.NamingDateName(vtd) + ".txt", IOsavedform, INsavedform, 2);
                    in = new Scanner(new File(ITMadd + "/" + icode.getText() + "/Status/" + ShortFunction.NamingDateName(vtd) + ".txt"), "UTF-8");
                    in.nextLine();
                    String p = in.nextLine();
                    float stck = Float.valueOf(p) - striQTY + Float.valueOf(iqty.getText());
                    // TextIO.putln("p:;"+p+"stck:"+stck);
                    ShortFunction.ReplaceLineWN(ITMadd + "/" + icode.getText() + "/Status", ShortFunction.NamingDateName(vtd) + ".txt", 2, String.valueOf(stck));
                    in.close();

                    dsd = ShortFunction.NamingDate(vtd);
                    for (dsd = ShortFunction.Date_plus(dsd); ShortFunction.DateInBetween(dsd, dsd, Eyear); dsd = ShortFunction.Date_plus(dsd)) {                               //Updating all file created or having date of early created
                        tmp = new File(ITMadd + "/" + icode.getText() + "/Status/" + ShortFunction.NamingDateName(dsd) + ".txt");
                        bg = tmp.exists();
                        if (bg) {
                            //  TextIO.putln(ITMadd+"/"+icode.getText()+"/Status/"+ShortFunction.NamingDateName(dsd)+".txt");
                            in = new Scanner(tmp, "UTF-8");
                            ...
                            ShortFunction.ReplaceLineWN(ITMadd + "/" + icode.getText() + "/Status", ShortFunction.NamingDateName(dsd) + ".txt", 1, String.valueOf(stck));
                            in.close();
                        }
                    }
                    ...
                    JLabel label = new JLabel("Customer data storing date. not found or opens !!! Error0011");
            ...
            /* try {
             ...
             Logger.getLogger(Customer_Edit.class.getName()).log(Level.SEVERE, null, ex);
             }*/

            if (ShortFunction.repeatation(CMRadd + "/" + ccode.getText() + STRaddG + "/", ShortFunction.NamingDateName(vtd) + ".txt", COsavedform) == 1 && ShortFunction.repeatation(ITMadd + "/" + icode.getText() + "/Status", ShortFunction.NamingDateName(vtd) + ".txt", IOsavedform) == 1) {
                try {
                    Scanner in;
                    //if(repeatationWOcase.mainn(CMRadd +"/" + ccode.getText()+ STRaddG,  ShortFunction.NamingDateName(vtd)+".txt", COsavedform)==0&&repeatationWOcase.mainn(ITMadd+"/"+icode.getText()+"/Status", ShortFunction.NamingDateName(vtd)+".txt", IOsavedform)==0)
                    //  System.out.println("You Can ..");
                    ShortFunction.ReplaceLineEx(CMRadd + "/" + ccode.getText() + STRaddG + "/", ShortFunction.NamingDateName(vtd) + ".txt", COsavedform, CNsavedform, 2);//ShortFunction.NamingDateName(vtd)
                    in = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddG + "/" + ShortFunction.NamingDateName(vtd) + ".txt"), "UTF-8");
                    in.nextLine();
                    String pd = in.nextLine();
                    Float nd = Float.valueOf(pd) - striAMT + tot;
                    System.out.println("Updating Total");
                    ShortFunction.ReplaceLineWN(CMRadd + "/" + ccode.getText() + STRaddG, ShortFunction.NamingDateName(vtd) + ".txt", 2, String.valueOf(nd));//ShortFunction.NamingDateName(vtd)
                    in.close();
                    String dsd = ShortFunction.NamingDate(vtd);
...
//###################################################################################################################################    
    ...
    public void CMRxName() {
        if (ShortFunction.repeatation(CMRadd, "customer.txt", ccode.getText()) == 1) {          //Customer's account exists or not  ####111
            File tmp = new File(CMRadd + "/" + ccode.getText() + "/profile.txt");
            boolean ex = tmp.exists();
            if (ex) {                                                                 //Check for name of customer
                Scanner read;
                try {
                    read = new Scanner(new File(CMRadd + "/" + ccode.getText() + "/profile.txt"), "UTF-8");
                    read.nextLine();
                    cname.setText(read.nextLine());
                    read.close();
                } catch (FileNotFoundException ex1) {
                    JLabel label = new JLabel("Customer profile.not found !!! Error0017");
                    label.setFont(new Font("Arial", Font.BOLD, 20));
                    JOptionPane.showMessageDialog(null, label, "ERROR", JOptionPane.WARNING_MESSAGE);
                }
            } else {
...
//######################################################################################################################################3
    ...
    public void see() {
        ...
        //check in Den
        String ndt = ShortFunction.NamingDate(dt1);
        while (ShortFunction.DateInBetween(ShortFunction.NamingDate(dt1), ndt, ShortFunction.NamingDate(dt2))) {

            Scanner read;
            DefaultTableModel model = (DefaultTableModel) table.getModel();
            String td;
            File tmp = new File(CMRadd + "/" + ccode.getText() + STRaddG + "/" + ShortFunction.NamingDateName(ndt) + ".txt");
            boolean ex = tmp.exists();
            if (ex) {
                //System.out.println(CMRadd+"/"+ccode.getText()+STRaddG+"/"+ShortFunction.NamingDateName(ndt)+".txt");
                try {
                    read = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddG + "/" + ShortFunction.NamingDateName(ndt) + ".txt"), "UTF-8");
                    read.useDelimiter(",|\\n");
                    read.nextLine();
                    read.nextLine();
                    while (read.hasNext()) {
                        td = read.next();
                        if (ShortFunction.DateInBetween(dt1, td, dt2)) {
                            itemcode = read.next();
                            itemname = read.next();
                            itemqty = read.next();
                            itemrate = read.next();
            ...
            tmp = new File(CMRadd + "/" + ccode.getText() + STRaddT + "/" + ShortFunction.NamingDateName(ndt) + ".txt");
            ex = tmp.exists();
            if (ex) {
                try {
                    read = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddT + "/" + ShortFunction.NamingDateName(ndt) + ".txt"), "UTF-8");
                    read.useDelimiter(",|\\n");
                    read.nextLine();
                    while (read.hasNext()) {
                        td = read.next();
            ...
            ndt = ShortFunction.Date_plus(ndt);                                       // Updation in date interval
            //TextIO.putln(ndt+"  "+ShortFunction.DateInBetween(ShortFunction.NamingDate(dt1), ndt,ShortFunction.NamingDate(dt2) ));
        }
    ...
    private void tableKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tableKeyPressed
        ...
        if (table.getSelectedRow() >= 0) {
            if (b == 69) {      //For Edit  "e"
                ...
                if (way.equals("Given")) {
                    IOsavedform = vtd + "," + ccode.getText() + ",-" + iqty.getText() + "," + irate.getText() + "," + iamt.getText() + "," + mode + ",Customer";
                        ...
                        if (editt >= 0) {
                            ...
                            if (way.equals("Given")) {
                                try {
                                    ...
                                    Scanner in;
//###############################################################################################################################################################3                              
                                    //ShortFunction.NamingDateName(vtd)
                                    //System.out.println("ShortFunction.NamingDateName(vtd): "+ShortFunction.NamingDateName(vtd));
                                    //System.out.println(CMRadd +"/" + ccode.getText()+ STRaddG+"/"+ShortFunction.NamingDateName(vtd)+".txt\n"+Csavedform);
                                    String st1 = vtd + "," + ccode.getText() + ",-" + striqty + "," + strirate + "," + striamt;
                                    String st2 = "," + mode + ",Customer";
                                    String Isavedform = st1 + st2;
                                    if (ShortFunction.repeatation(CMRadd + "/" + ccode.getText() + STRaddG, ShortFunction.NamingDateName(vtd) + ".txt", Csavedform) == 1 && ShortFunction.repeatation(ITMadd + "/" + stricode + "/Status", ShortFunction.NamingDateName(vtd) + ".txt", Isavedform) == 1) {
                                        det = true;
                                        ShortFunction.RemoveLineEx(CMRadd + "/" + ccode.getText() + STRaddG, ShortFunction.NamingDateName(vtd) + ".txt", Csavedform, 2);
                                        in = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddG + "/" + ShortFunction.NamingDateName(vtd) + ".txt"), "UTF-8");
                                        in.nextLine();
                                        String pd = in.nextLine();
                                        ...
                                        for (dsd = ShortFunction.Date_plus(dsd); ShortFunction.DateInBetween(dsd, dsd, Eyear); dsd = ShortFunction.Date_plus(dsd)) {
                                            ...
                                            bg = tmp.exists();
                                            if (bg) {
                                                in = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddG + "/" + ShortFunction.NamingDateName(dsd) + ".txt"), "UTF-8");
                                                pd = in.nextLine();
                                                nd = Float.valueOf(pd) - Float.valueOf(striamt);
                                                ShortFunction.ReplaceLineWN(CMRadd + "/" + ccode.getText() + STRaddG, ShortFunction.NamingDateName(dsd) + ".txt", 1, String.valueOf(nd));
                                                in.close();
                                            }
                                        }

//###################################################################################################################################################################
                                        ShortFunction.RemoveLineEx(ITMadd + "/" + stricode + "/Status", ShortFunction.NamingDateName(vtd) + ".txt", Isavedform, 2);                      // Update current file of status

                                        tmp = new File(ITMadd + "/" + stricode + "/Status/" + ShortFunction.NamingDateName(vtd) + ".txt");
                                        bg = tmp.exists();
                                        if (bg) {
                                            in = new Scanner(tmp, "UTF-8");
                                            in.nextLine();
                                            ...
                                            ShortFunction.ReplaceLineWN(ITMadd + "/" + stricode + "/Status", ShortFunction.NamingDateName(vtd) + ".txt", 2, String.valueOf(stck));
                                            in.close();
                                        }

                                        dsd = ShortFunction.NamingDate(vtd);
                                        for (dsd = ShortFunction.Date_plus(dsd); ShortFunction.DateInBetween(dsd, dsd, Eyear); dsd = ShortFunction.Date_plus(dsd)) {                               //Updating all file created or having date of early created
                                            tmp = new File(ITMadd + "/" + stricode + "/Status/" + ShortFunction.NamingDateName(dsd) + ".txt");
                                            bg = tmp.exists();
                                            if (bg) {
                                                // TextIO.putln(ITMadd+"/"+icode.getText()+"/Status/"+ShortFunction.NamingDateName(dsd)+".txt");
                                                in = new Scanner(tmp, "UTF-8");
                                                String p = in.nextLine();
                                                float stck = Float.valueOf(p) + Float.valueOf(striqty);
                                                ShortFunction.ReplaceLineWN(ITMadd + "/" + stricode + "/Status", ShortFunction.NamingDateName(dsd) + ".txt", 1, String.valueOf(stck));
                                                in.close();
                                            }
                                        }
                                    } else {
...
//###############################################################################################################################################################3                                                          
                                    ...
                                    JLabel label = new JLabel("Customer data storing date. not found or opens.. some how !!! Error0011");
                                    label.setFont(new Font("Arial", Font.BOLD, 20));
                                    JOptionPane.showMessageDialog(null, label, "ERROR", JOptionPane.WARNING_MESSAGE);
                                }
                            } else {// Way "Taken"
...
//###################################################################################################################################################################
                                    if (ShortFunction.repeatation(CMRadd + "/" + ccode.getText() + STRaddT, ShortFunction.NamingDateName(vtd) + ".txt", Csavedform) == 1) {
                                        det = true;
                                        ShortFunction.RemoveLineEx(CMRadd + "/" + ccode.getText() + STRaddT, ShortFunction.NamingDateName(vtd) + ".txt", Csavedform, 1);
                                        Scanner in;
                                        in = new Scanner(new File(CMRadd + "/" + ccode.getText() + STRaddT + "/" + ShortFunction.NamingDateName(vtd) + ".txt"), "UTF-8");
                                        String pd = in.nextLine();
                                        Float nd = Float.valueOf(pd) - Float.valueOf(striamt);
